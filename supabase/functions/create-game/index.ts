...
// Cache della chiave pubblica ES256 di Supabase
...
async function verifyToken(authHeader: string): Promise<string> {
  ...
  // verify() lancia eccezione se firma non valida o token scaduto
...
serve(async (req) => {
  ...
  try {
    ...
    if (!title || !title.trim()) {
      return new Response(JSON.stringify({ error: "Il titolo è obbligatorio" }), { status: 400, headers: corsHeaders });
    }
    ...
    if ((count ?? 0) >= 10) {
      return new Response(JSON.stringify({ error: "Limite massimo di 10 partite attive raggiunto" }), { status: 400, headers: corsHeaders });
    }
